      ...
      <ContentArea>
        <DashboardShell>
          <div className="col-span-full xl:col-span-8 flex flex-col gap-6">
            <div className="flex items-center justify-between">
              ...
              <button 
                ...
                className="flex items-center gap-2 px-5 py-2.5 bg-white border border-[#E2E8F0] hover:bg-gray-50 hover:border-gray-300 text-[#1B1D35] rounded-2xl transition-all shadow-sm font-semibold text-[14px]"
              >
          ...
          <div className="col-span-full xl:col-span-4 flex flex-col gap-5 pt-0 xl:pt-[84px]">
             <div className="bg-white p-6 rounded-3xl text-[#1B1D35] shadow-sm border border-[#E2E8F0]">
                <div className="flex items-center gap-2 mb-4 text-[#6C5CE7]">
                  <Zap className="w-5 h-5 fill-current" />
                  <h3 className="text-[16px] font-bold text-[#1B1D35] uppercase tracking-wider">Consistency Insights</h3>
                </div>
                <p className="text-[#4A5568] text-[14px] leading-relaxed">Completing study plan tasks on time increases concept mastery retention by up to 34%.</p>
                
                <div className="mt-8 flex justify-between items-end px-2">
                  {['M','T','W','T','F','S','S'].map((day, i) => (
                    <div key={i} className="flex flex-col items-center gap-2">
                      <div 
                        className={`w-7 rounded-full transition-all ${i < 4 ? 'bg-gradient-to-t from-[#6C5CE7] to-[#8B7CF6]' : 'bg-gray-100'}`} 
                        style={{ height: i < 4 ? `${40 + i * 8}px` : '24px' }} 
                      />
                      <span className="text-[12px] text-[#718096] font-bold">{day}</span>
                    </div>
                  ))}
                </div>
             </div>
